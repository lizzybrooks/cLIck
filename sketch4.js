...
function signInWithGoogle() {
    ...
    firebase.auth().signInWithPopup(provider).then((result) => {
        ...
        // Update or store this user data
...
function updateUserList(userData) {
    if (!data.users) {
        data.users = []; // Ensure the users array is initialized
    }

    let userExists = data.users.some(u => u.uid === userData.uid);
    if (!userExists) {
        data.users.push(userData);
    } else {
        let userIndex = data.users.findIndex(u => u.uid === userData.uid);
...
function search() {
    console.log(data.users); // Check what 'data' contains
    if (!data || !data.users) {
        console.error('Data or users array is undefined.');
...
function displaySearchResults(results) {
    console.log('Displaying results:', results);  // Log the results array to see what is being processed
    ...
    if (results.length === 0) {
        ...
        results.forEach(user => {
            let listItem = createElement('li', user.name);
            console.log('Appending:', listItem);  // Log each item to see if it's being created
    ...
    if (user) {
        ...
        storageRef.put(file).then((snapshot) => {
            ...
            snapshot.ref.getDownloadURL().then((downloadURL) => {
                ...
                saveImageMetadata(user.uid, file.name, downloadURL);
            });
        }).catch((error) => {
...
function saveImageMetadata(userId, fileName, imageUrl) {
    // Encode the fileName to a Firebase-friendly format
    const encodedFileName = btoa(fileName).replace(/=/g, ''); // Base64 encode and remove '='

    const metadata = {
        imageUrl: imageUrl,
        uploadedAt: firebase.database.ServerValue.TIMESTAMP // Adds a timestamp
    };

    const dbRef = firebase.database().ref('images/' + userId + '/' + encodedFileName);
    dbRef.set(metadata).then(() => {
